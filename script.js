...
function createName() {
  if (!document.getElementById("page1").classList.contains("active")) return; // 🛑 only page1
  ...
  if (texts.length > 20) texts[0].remove();
}
...
setTimeout(() => {
  // start floating names only once
  nameInterval = setInterval(createName, 700);
}, 500);
...
function goToPage3() {
  ...
  // 🛑 Stop heavy intervals from page2
...
function sunflowerLoop() {
  ...
  sunflowerInterval = setInterval(() => {
    ...
    sun.style.animationDuration =
      (window.innerWidth < 500 ? 2.5 + Math.random() * 1.5 : 3 + Math.random() * 2) + "s";
    ...
    if (flowers.length > 20) flowers[0].remove();
  }, 600);
}
